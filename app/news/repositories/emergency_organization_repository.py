...
class EmergencyOrganizationRepository(EmergencyOrganizationRepositoryInterface):
    ...
    def find_similar(
        ...
        # the same way via a correlated unnest, mirroring the multi-alias
        # scoring in condition_repository.find_similar().
        ...
        alias_value = func.unnest(EmergencyOrganization.aliases).table_valued("value")
        alias_score = (
            select(
                func.coalesce(
                    func.max(
                        func.greatest(
                            func.word_similarity(
                                normalize_arabic_sql(alias_value.c.value),
                                normalized_text,
                            ),
                            func.word_similarity(
                                normalize_arabic_sql(
                                    alias_value.c.value, compact=True
                                ),
                                compact_text,
                            ),
                        )
