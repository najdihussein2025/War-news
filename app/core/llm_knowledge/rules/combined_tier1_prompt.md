...
- Only explicit route/path wording (`طريق X - Y`, `طريق عام X - Y`, or `طريق بين X و Y`) names two endpoints. Capture both endpoints as separate target entries and in the same `sub_events.locations`. Plain `بين X و Y`, `بين بلدتي X و Y`, and `في المنطقة الواقعة بين X و Y` without a road/route are one fuzzy target attributed to the first village with the other village kept as review context. Example: «استهدف دراجة نارية على طريق عام مرج حاروف - زبدين» → village=["حاروف","زبدين"].
